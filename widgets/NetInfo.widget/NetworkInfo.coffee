...
command: "NetInfo.widget/NetworkInfo.sh"
...
update: (output, domEl) -> 
  ...
  for svc in data.service
    ...
    if svc.ipaddress == ''
      html += "  <img class='icon' src='NetInfo.widget/images/" + svc.name + "_disabled.png'/>"
      html += "  <p class='primaryInfo'>Not Connected</p>" 
    else
      html += "  <img class='icon' src='NetInfo.widget/images/" + svc.name + ".png'/>"
